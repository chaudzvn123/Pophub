...
# ================== CẤU HÌNH ==================
PREFIX = ","
ADMIN_UID = [1265245644558176278]  # Thay bằng Discord ID của bạn
...
async def tai_xiu_game(ctx, choice: str, amount: int):
    ...
    if is_win:
        embed.add_field(name="Tiền thắng", value=f"+{win_amount:,} xu", inline=False)
    else:
        embed.add_field(name="Tiền thua", value=f"{win_amount:,} xu", inline=False)
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        ...
        await ctx.send(f"❌ Lệnh không tồn tại! Dùng `{PREFIX}help` để xem danh sách.")

# ================== RUN BOT ==================
try:
    token = input("🔑 Nhập Discord Bot Token: ").strip()
    if not token:
        raise Exception("❌ Bạn chưa nhập token!")
    bot.run(token)
except discord.HTTPException as e:
    if e.status == 429:
        print("❌ Bị chặn kết nối do spam request (Rate Limited).")
    else:
        raise e
